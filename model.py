...
def build_model(batch_size=1, num_cp=6):
    ...
    x = MaxPooling3D((4, 4, 4))(x)
    x = Reshape((1, -1))(x)
    x = Conv1D(4 * 128, 3, activation='relu', padding='same', kernel_initializer="he_normal")(x)
    x = Conv1D(8 * 128, 3, activation='relu', padding='same', kernel_initializer="he_normal")(x)
    x = Conv1D(16 * 128, 3, activation='relu', padding='same', kernel_initializer="he_normal")(x)
    x = Conv1D(num_cp * 128, 3, activation='relu', padding='same', kernel_initializer="he_normal")(x)
    x = Conv1D(num_cp * 128, 3, activation='relu', padding='same', kernel_initializer="he_normal")(x)
    latent_space = x
...
def monaco_plan(latent_space, num_cp):
    ...
    mus = []
    mu_total = Conv1D(num_cp * 32, 3, activation='relu', padding='same', kernel_initializer="he_normal")(latent_space)
    mu_total = Conv1D(num_cp * 8, 3, activation='relu', padding='same', kernel_initializer="he_normal")(mu_total)
    mu_total = Conv1D(num_cp, 1, activation='relu', padding='same', kernel_initializer="he_normal")(mu_total)
